...
module.exports = function (source) {
  ...
  let head = {
    ...
    ignoreSuffixChar: opts.options.ignoreSuffixChar || ''
  }
  ...
  if (navRs && navRs[1]) {
    navRs[1].split('\n').filter(one => {
      ...
      let list = one.split(':')
      if (list[0] === 'title') {
        head.title = trim(list[1])

        if (head.ignoreSuffixChar) {
          if (head.title.indexOf(head.ignoreSuffixChar) > 0) {
            // do nothing
          } else {
            head.title += opts.options.titleSuffix
          }
        } else {
          head.title += opts.options.titleSuffix
        }
